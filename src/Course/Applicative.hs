...
--   `∀u y. u <*> pure y = pure ($ y) <*> u` -- TODO : grok this
...
instance Applicative List where
  ...
  (<*>) Nil _ = Nil
  (<*>) (f:.fs) xs = (f<$>xs)++(fs<*>xs) -- TODO spaceleak..
...
lift1 = (<$>)
-- lift1 f x = (pure f) <*> x -- just to follow the pattern
...
filtering _ Nil = pure Nil -- TODO: Figure out the idiomatic solution.
filtering f (x:.xs) = ((++) <$> (\b->if b then x:.Nil else Nil)<$>(f x))<*>(filtering f xs)
