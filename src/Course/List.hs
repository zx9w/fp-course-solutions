...
sum ::
  List Int
  -> Int
sum xs = foldRight (+) 0 xs -- should be using foldMap for monoids.
...
length ::
  List a
  -> Int
length xs = foldRight (\_ y -> y+1) 0 xs
...
(++) Nil ys = ys
(++) (x:.Nil) ys = x:.ys
(++) (x:.xs) ys = x:.(xs ++ ys) -- TODO improve this
...
flatMap ::
  ...
  -> List b
flatMap _ Nil = Nil
flatMap f (x:.xs) = (f x) ++ (flatMap f xs) -- TODO: spaceleak, not tail recursive
...
reverse ::
  ...
  -> List a
reverse Nil = Nil
reverse (x:.xs) = (reverse xs) ++ (x:.Nil) -- TODO foldl
