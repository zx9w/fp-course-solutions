...
instance Functor (State s) where
  ...
  (<$>) f (State g) = (State (\s -> (f (eval (State g) s), exec (State g) s)))
...
instance Applicative (State s) where
  ...
  -- This is an evil beast from hell
  (<*>) :: State s (a -> b) -> State s a -> State s b
  (<*>) (State f) (State g) = State $ \s ->
    ((eval (State f) s) (eval (State g) s), (exec (State g) (exec (State f) s)))
...
instance Monad (State s) where
  ...
  (=<<) f (State g) = State $ \s ->
    (eval (f (eval (State g) s)) s, (exec (f $ eval (State g) s) (exec (State g) s)))
...
findM f (x:.xs) = (f x) >>=
  (\b-> if b then (Full x) else join . (findM f xs))
